/*************************************************************************
	...
	Raspberry Pi Pico baremetal C/C++ SDK port by hspil
	V1.3 1/21/2023
...
/*
   Contructor, simple constructor, it initiates the serial port
*/
void IC746::begin() {
	stdio_init_all();
	do {
...
// PTT
void IC746::addCATPtt(void (*userFunc)(bool)) {
	catSetPtt = userFunc;
}

// Split
void IC746::addCATsplit(void (*userFunc)(bool)) {
	catSplit = userFunc;
}
...
void IC746::addCATGetMode(uint8_t (*userFunc)(void)) {
	catGetMode = userFunc;
}

// GetPTT - function must return true for Tx and false for Rx
void IC746::addCATGetPtt(bool (*userFunc)(void)) {
	catGetPtt = userFunc;
}


// S meter (user function must return 0-15)
void IC746::addCATSMeter(uint8_t (*userFunc)(void)) {
...
void IC746::addCATMSet(void (*userFunc)(uint8_t)) {
	catSetMode = userFunc;
}

// Set VFOA or B - user function must interpret VFO per the constants in IC746.h
void IC746::addCATVSet(void (*userFunc)(uint8_t)) {
	catSetVFO = userFunc;
}
...
//
void IC746::send(uint8_t *buf, int len) {
	int i;

	putchar(CAT_PREAMBLE);
	putchar(CAT_PREAMBLE);
...
// sendResponse
// 
void IC746::sendResponse(uint8_t *buf, int len) {
...
//
void IC746::sendAck() {
	uint8_t ack[] = {CAT_RIG_ADDR, CAT_CTRL_ADDR, CAT_ACK};
	send(ack, 3);
}
...
//
void IC746::sendNack() {
	uint8_t nack[] = {CAT_RIG_ADDR, CAT_CTRL_ADDR, CAT_NACK};
...
bool IC746::readCmd() {
	...
	while (stdio_usb_connected() && !cmdRcvd) {
		...
		switch (rcvState) {
			...
			case CAT_RCV_INIT:      // check for second preamble uint8_t
				if (bt == CAT_PREAMBLE) {
...
void IC746::FreqtoBCD(long freq) {
	uint8_t ones, tens, hund, thou, ten_thou, hund_thou, mil, ten_mil, hund_mil, thou_mil;

	ones =     uint8_t(freq % 10);
	tens =     uint8_t((freq / 10L) % 10);
	cmdBuf[CAT_IX_FREQ] = uint8_t((tens << 4)) | ones;

	hund =      uint8_t((freq / 100L) % 10);
	thou =      uint8_t((freq / 1000L) % 10);
	cmdBuf[CAT_IX_FREQ + 1] = uint8_t((thou << 4)) | hund;

	ten_thou =  uint8_t((freq / 10000L) % 10);
	hund_thou = uint8_t((freq / 100000L) % 10);
	cmdBuf[CAT_IX_FREQ + 2] = uint8_t((hund_thou << 4)) | ten_thou;

	mil =       uint8_t((freq / 1000000L) % 10);
	ten_mil =   uint8_t(freq  / 10000000L) % 10; 
	cmdBuf[CAT_IX_FREQ + 3] = uint8_t((ten_mil << 4)) | mil;

	hund_mil = uint8_t((freq / 100000000L) % 10);
	thou_mil = uint8_t(freq  / 1000000000L % 10); // Allow frequencies up to 9999.999999 MHz to be enterd
	cmdBuf[CAT_IX_FREQ + 4] = uint8_t((thou_mil << 4)) | hund_mil;
}

void IC746::SmetertoBCD(uint8_t s) {
	uint8_t ones, tens, hund;

	ones =     uint8_t(s % 10);
	tens =     uint8_t((s / uint8_t(10)) % 10);
	cmdBuf[CAT_IX_SMETER+1] = uint8_t((tens << 4)) | ones;
