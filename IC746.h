...
#define CAT_RCV_WAITING     0  // waiting for 1st preamble uint8_t
#define CAT_RCV_INIT        1  // waiting for 2nd preamble uint8_t
#define CAT_RCV_RECEIVING   2  // waiting for command uint8_ts

// Command buffer (without preamble and EOM)
// |FE|FE|56|E0|cmd|sub-cmd|data|FD|  // Preamble (FE) and EOM (FD) are discarded leaving
// 2 addr uint8_ts , 1 command, 1 sub-command, up to 12 data, (longest is unimplemented edge frequency)
...
typedef void (*FuncPtrBoolean)(bool);
typedef bool (*FuncPtrVoidBoolean)(void);
